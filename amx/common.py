...
from calls import gmx,get_machine_config,move_file
...
def extract_itp(topfile):
	...
	with open(state.here+topfile,'r') as f: topfile = f.read()
	chains = {}
	...
	with open(state.here+'protein.itp','w') as fp: 
		for line in topfile.split('\n'):
			...
			if not re.match(".+forcefield\.itp",line) and not \
				...
				fp.write(line+'\n')
	if 'itp' not in state: state.itp = ['protein.itp']
	else: state.itp.append('protein.itp')
...
def counterions(structure,top=None,includes=None,ff_includes=None,gro='counterions'):
	...
	gmx('genion',base='genion',gro=gro,ndx='solvate-waters',
		cation=state.q('cation'),anion=state.q('anion'),
